...
public final class IntentUtils {
    ...
    public static Intent openPlayStore(final Context context, final boolean openInBrowser) {
        ...
        final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id="
                + appPackageName));
     ...
     * @return the intent
     * @see #sendEmail(String[], String, String) #sendEmail(String[], String, String)
     */
    ...
    public static Intent sendSms(final Context context, final String to, final String message) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            final String defaultSmsPackageName = Telephony.Sms.getDefaultSmsPackage(context);
            final Intent intent = new Intent(Intent.ACTION_SENDTO, Uri.parse("smsto:" + to));
            ...
            final Uri smsUri = Uri.parse("tel:" + to);
     ...
     *                  A zoom of 1.0 is 90 degree horizontal FOV for a nominal landscape mode 4 x 3
     *                  aspect ratio display Android
    ...
    public static Intent showStreetView(final float latitude,
        ...
        final StringBuilder builder = new StringBuilder("google.streetview:cbll=").append(latitude)
                .append(",").append(longitude);
        if (yaw != null || pitch != null || zoom != null) {
            final String cbpParam = String.format("%s,,%s,%s", yaw == null ? ""
                    : yaw, pitch == null ? "" : pitch, zoom == null ? "" : zoom);
     ...
     * @param url the url
     * @return the intent
     * @see #openLink(String) #openLink(String)
     */
     ...
     * @param file the file
     * @return the intent
     * @see #openVideo(File) #openVideo(File)
     */
     ...
     * @param file the file
     * @return the intent
     * @see #openAudio(File) #openAudio(File)
     */
     ...
     * @param file the file
     * @return the intent
     * @see #openImage(String) #openImage(String)
     */
     ...
     * @param file the file
     * @return the intent
     * @see #openText(String) #openText(String)
     */
     ...
     * {         Uri file = data.getData();     } </code></pre>
     */
     ...
     *              <p/>
     *              <code><pre>
     *              <p> // Select only from users with emails
     ...
     * @see #cropImage(Context, File, int, int, int, int, boolean) #cropImage(Context, File, int, int, int, int, boolean)
     */
     ...
     * @return Intent with <code>data</code>-extra in <code>onActivityResult</code>
     * which contains result as a {@link android.graphics.Bitmap}. See demo app for details
     */
    public static Intent cropImage(final Context context, final File image, final int outputX
            , final int outputY, final int aspectX, final int aspectY, final boolean scale) {
        final Intent intent = new Intent("com.android.camera.action.CROP");
        intent.setType("image/*");
     ...
     * openMedia
     *
     * @param uri      , Uri instance :  Builds and parses URI references which conform to RFC 2396
    ...
    /**
     * isSupportsContactsV2
     *
     * @return whether sdk is greater than Eclair
     */
